...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  if (info.menuItemId === "ss_media" && info.srcUrl) return quickDownload(info.srcUrl, ref, tab && tab.title);
  if (info.menuItemId === "ss_link" && info.linkUrl) return quickDownload(info.linkUrl, ref, tab && tab.title);
  // page: open the app on the page URL so the user can choose (or the popup lists items)
  openApp((tab && tab.url) || "");
});
